...
class TestFeatureGeneration:
    ...
    def test_feature_matrix_has_correct_row_count(self):
        ...
        assert len(matrix) == len(meta["candidates"]), (
            ...
            f"candidates ({len(meta['candidates'])})"
        )
        ...
        for r1, r2 in zip(matrix1, matrix2):
            for key in FEATURE_NAMES:
                assert r1[key] == r2[key], (
                    f"Non-deterministic feature {key}: {r1[key]} != {r2[key]}"
                )
...
class TestQueryTimeCutoff:
    ...
    def test_no_post_complaint_tx_in_features(self):
        ...
        for case_raw in meta["cases_raw"]:
            ...
            case_txs = [
                t for t in meta["transactions"]
                if t["case_id"] == case_raw["case_id"]
            ]
            post_txs = [
                t for t in case_txs
    ...
    def test_tx_count_matches_pre_complaint(self):
        ...
        for row in matrix:
            ...
            pre_txs = [
                t for t in meta["transactions"]
                if t["case_id"] == row["case_id"]
                and datetime.fromisoformat(t["timestamp"]) <= cutoff
            ]
            assert row["tx_count"] == len(pre_txs), (
                f"Case {row['case_id']}: tx_count={row['tx_count']} "
                f"but actual pre-TX count={len(pre_txs)}"
            )
    ...
    def test_tx_total_amount_matches_pre_complaint(self):
        ...
        for row in matrix[:20]:  # check first 20
            ...
            assert abs(row["tx_total_amount"] - expected_total) < 0.01, (
                f"Case {row['case_id']}: tx_total_amount={row['tx_total_amount']} "
...
class TestTargetIsolation:
    ...
    def test_no_ground_truth_in_features(self):
        ...
        forbidden_patterns = [
            "true_location", "actual_cashout", "cashout_time",
            "cashout_metro", "target_distance", "gt_distance",
        ]
        for fname in FEATURE_NAMES:
            for pattern in forbidden_patterns:
                assert pattern.lower() not in fname.lower(), (
                    f"Feature '{fname}' contains forbidden pattern '{pattern}'"
                )
...
class TestFutureEventIsolation:
    ...
    def test_no_cashout_time_in_features(self):
        ...
        forbidden_exact = [
            "cashout_time", "cashout_metro", "cashout_location",
            "actual_cashout", "true_cashout",
        ]
        for fname in FEATURE_NAMES:
            for forbidden in forbidden_exact:
                assert forbidden.lower() not in fname.lower(), (
                    f"Feature '{fname}' references forbidden '{forbidden}'"
                )
    ...
    def test_case_scenario_features_are_binary(self):
        ...
        for row in matrix:
            for sf in scenario_features:
                assert row[sf] in (0, 1), (
                    f"Feature {sf} = {row[sf]}, expected 0 or 1"
                )
...
class TestCandidateLevelCorrectness:
    ...
    def test_binary_features_are_zero_or_one(self):
        ...
        binary_features = [
            ...
            if f.startswith("loc_type_") or f.startswith("cand_same_")
            or f.startswith("loc_is_") or f.startswith("case_scenario_")
        ]
        for row in matrix:
            for bf in binary_features:
                assert row[bf] in (0, 1), (
                    f"{bf} = {row[bf]}, expected 0 or 1"
                )
    ...
    def test_one_scenario_active_per_case(self):
        ...
        for row in matrix:
            ...
            assert active == 1, (
                f"Case {row['case_id']}: {active} scenarios active, expected 1"
            )
    ...
    def test_same_seed_same_features(self):
        ...
        for i, (r1, r2) in enumerate(zip(matrix1, matrix2)):
            for key in FEATURE_NAMES + METADATA_COLUMNS:
                assert r1[key] == r2[key], (
                    f"Row {i}, feature {key}: {r1[key]} != {r2[key]}"
                )
    ...
    def test_case_level_features_consistent_across_candidates(self):
        ...
        for case_id, rows in by_case.items():
            for cf in case_features:
                ...
                assert len(values) == 1, (
                    f"Case {case_id}, feature {cf}: "
                    f"multiple values {values} across candidates"
                )
        ...
        for row in matrix:
            ...
            if not pre_txs:
                for sf in sentinel_features:
                    assert row[sf] == -1.0, (
                        f"Case {row['case_id']}: {sf} = {row[sf]}, "
                        f"expected -1.0 (no pre-complaint TX)"
                    )
        ...
        for entry in FEATURE_REGISTRY:
            for field in required:
                assert field in entry, (
                    f"Registry entry {entry.get('name')}: missing field '{field}'"
                )

    def test_all_features_allowed(self):
        """No feature should be marked as FORBIDDEN."""
        for entry in FEATURE_REGISTRY:
            assert entry["leakage"] == "ALLOWED", (
                f"Feature {entry['name']}: leakage={entry['leakage']}"
            )

    def test_all_features_query_time(self):
        """All features should be available at query time."""
        for entry in FEATURE_REGISTRY:
            assert entry["query_time"] is True, (
                f"Feature {entry['name']}: query_time={entry['query_time']}"
            )
