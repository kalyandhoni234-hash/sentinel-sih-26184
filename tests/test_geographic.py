...
def test_locations_have_valid_coordinates():
    ...
    with open(locs_path) as f:
        for line in f:
            loc = json.loads(line)
            assert -90 <= loc["latitude"] <= 90, f"Location {loc['location_id']}: invalid latitude {loc['latitude']}"
...
def test_candidate_distances_are_non_negative():
    ...
    with open(cands_path) as f:
        for line in f:
            c = json.loads(line)
            assert c["distance_from_origin_km"] >= 0, f"Candidate has negative distance: {c['distance_from_origin_km']}"
