...
from .scenarios import get_scenario_behavior
...
def _generate_hard_negatives(
    ...
    same_metro = [loc for loc in locations if loc.metro == case.origin_metro and loc.location_id != true_location.location_id]
    ...
    nearby = find_nearest_locations(
        true_location.latitude, true_location.longitude, locations, k=15
    )
    close_wrong_type = [
        loc for loc, dist in nearby
        if loc.location_id != true_location.location_id
        and loc.location_type != true_location.location_type
        and dist < 10
    ]
    hard_negatives.extend(rng.sample(
        close_wrong_type,
    ...
    if same_metro:
        far_same_metro = [
            loc for loc in same_metro
            if compute_distance_km(
                true_location.latitude, true_location.longitude,
...
def generate_candidates_for_case(
    ...
    true_loc = next(
        ...
        if loc.location_id == true_ground_truth.actual_cashout_location_id
    )
    ...
    if origin_loc:
        true_dist = compute_distance_km(
            origin_loc.latitude, origin_loc.longitude,
            true_loc.latitude, true_loc.longitude,
        )
    ...
    candidates.append(Candidate(
        ...
        transaction_proximity_score=round(
            _compute_transaction_proximity_score(case, true_loc, locations), 4
        ),
    ...
    hard_negatives = _generate_hard_negatives(
        case, true_loc, locations, rng, hard_neg_count
    )
    for loc in hard_negatives:
        ...
        candidates.append(Candidate(
            ...
            transaction_proximity_score=round(
                _compute_transaction_proximity_score(case, loc, locations), 4
            ),
