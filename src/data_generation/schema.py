...
class Location(BaseModel):
    ...
    density_score: float = Field(
        ..., ge=0.0, le=1.0,
        description="Synthetic foot-traffic density score (0=low, 1=high)"
    )
    cash_out_attractiveness: float = Field(
        ..., ge=0.0, le=1.0,
        description="Synthetic attractiveness for cash-out activity"
    )
    is_high_surveillance: bool = Field(
        default=False,
        description="Whether the location has high surveillance (affects cash-out feasibility)"
    )
...
class Candidate(BaseModel):
    ...
    distance_from_origin_km: float = Field(
        ..., ge=0,
        description="Distance from complaint origin in km"
    )
    scenario_affinity: float = Field(
        ..., ge=0.0, le=1.0,
        description="How well this location fits the fraud scenario"
    )
    transaction_proximity_score: float = Field(
        ..., ge=0.0, le=1.0,
        description="Score based on transaction chain endpoint proximity"
    )
    temporal_plausibility: float = Field(
        ..., ge=0.0, le=1.0,
        description="Whether the timing is plausible for this location"
    )
    density_score: float = Field(
        ..., ge=0.0, le=1.0,
        description="Foot-traffic density at this location"
    )
    is_true_location: bool = Field(
        default=False,
        description="HIDDEN - only used in evaluation, never in model features"
    )
...
class GroundTruth(BaseModel):
    ...
    selection_probability: float = Field(
        ..., ge=0.0, le=1.0,
        description="Probability the generator assigned to this location"
    )
...
class DatasetManifest(BaseModel):
    ...
    scenario_distribution: dict[str, int] = Field(
        ...
        description="Count of cases per scenario type"
    )
